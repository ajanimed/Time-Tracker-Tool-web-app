...
class EmployeesTab extends Component {
  ...
  fetchEmployees(page,limit){
    ...
    instance.get('/employees/'+page+'/'+limit,options)
      .then(response=>{
        ...
        this.setState({employees:response.data.docs,pagination:paginationOptions,loadingTab:false});
  ...
  deleteEmployee(){
    ...
    instance.delete('/employee/delete/'+this.state._id,options)
      .then(response=>{
        console.log(response);
        let page = parseInt(this.state.pagination.page,10);
        if(this.state.pagination.totalDocs===1){
          this.fetchEmployees(page-1,this.props.limit);
        }
        else{
          this.fetchEmployees(page,this.props.limit);
        }
      })
      .catch(error=>{
        console.log(error);
              ...
              <CardBody>
                ...
                <Col md="12">
                  <div className="clearfix">
                    ...
                    <div className="reloadbtn"><Button onClick={()=>this.fetchEmployees()}  color="primary">Перезагружать</Button></div>
                    </div>
                  </div>
                </Col>):
                this.state.loadingTab ? (<div className="spinner">Loading...</div>) : ( this.state.employees.length === 0 ? (<div>No employees</div>) :(
                ...
                <Pagination className={this.state.togglePaginationClass}>
                  <PaginationItem>
                    <PaginationLink className={this.state.paginatePrevClass} onClick={()=>this.fetchEmployees(this.state.pagination.page-1,this.props.limit)}  previous tag="button" />
                  </PaginationItem>
                  {this.state.paginationTab.map(item  => (
                  <PaginationItem key={item.index} className={item.active} >
                   <PaginationLink tag="button" onClick={()=> this.fetchEmployees(item.index,this.props.limit)}>
                     {item.index}
                   </PaginationLink>
                  </PaginationItem>))}
                  <PaginationItem>
                    <PaginationLink className={this.state.paginateNextClass}  onClick={()=>this.fetchEmployees(this.state.pagination.page+1,this.props.limit)} next tag="button" />
                  </PaginationItem>
                </Pagination>)}
              </CardBody>
              <Modal isOpen={this.state.danger} toggle={()=>this.toggleDangerModal()}
                     className={'modal-danger ' + this.props.className}>
                <ModalHeader toggle={()=>this.toggleDangerModal()}>Удалить сотрудника</ModalHeader>
                <ModalBody>
                  Вы хотите удалить этого сотрудника?
                </ModalBody>
                <ModalFooter>
