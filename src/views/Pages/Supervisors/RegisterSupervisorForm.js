...
class RegisterSupervisorForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      formIsValid:false,
  ...
  updateRoleInput() {
    let newUser = this.state.user;
    newUser.supervisor.function = document.getElementById('role').value;
    this.setState({user: newUser});
  }
  ...
  registerSupervisor() {
    ...
    let params = {
      ...
      supervisor: {
        function: this.state.user.supervisor.function
      }
    };
    let options = {
      headers: {
    ...
    instance.post('/register', params, options)
      .then(response => {
        let success = {
          visible: true,
          message: 'руководител был добавлен'
        };
      ...
      <div className="animated fadeIn" >
        ...
        <Row>
          <Col xl={12}>
            <Card>
              <CardHeader>
                <strong>Зарегистрировать Руководителя</strong>
              </CardHeader>
              <CardBody>
                ...
                <Row style={{marginTop: '15px', paddingLeft: '25px', paddingRight: '10px'}}>
                  <Col xs="12">
                    <FormGroup>
                      ...
                      <Input onChange={() => this.updateTelInput()} className="col-sm-4" type="tel" id="tel" name="tel"
              ...
              <CardFooter>
                <Button type="submit"  onClick={() => this.registerSupervisor()} size="sm" color="success"><i
